...
@Injectable()
export class WebSocketAppDataGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    const request = context.switchToHttp().getRequest();
    console.log(request);
    console.log('received', request.handshake?.headers);
    const qs = request.handshake?.headers['tg-data']; // Assuming the data is in a query parameter named 'qs'
    if (!qs) {
